...
void makeWindow(screen_context_p nge_screen)
{
	WNDCLASSW winclass;
	int width;
	int height;
	...
	winclass.lpszClassName	= L"SINRGE2 Player";

	RegisterClassW(&winclass);

	width	= nge_screen->width + GetSystemMetrics(SM_CXFIXEDFRAME) * 2;
	height	= nge_screen->height + GetSystemMetrics(SM_CYFIXEDFRAME) * 2 + GetSystemMetrics(SM_CYCAPTION);

	rt.left		= (GetSystemMetrics(SM_CXSCREEN) - width) / 2;
	rt.top		= (GetSystemMetrics(SM_CYMAXIMIZED) - nge_screen->height) / 2 - GetSystemMetrics(SM_CYCAPTION);
	rt.right	= rt.left + width;
	rt.bottom	= rt.top + height;

	dwStyle = WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS;

	dwNum = MultiByteToWideChar(0, 0, nge_screen->name, -1, NULL, 0);
	wTitle = (wchar_t*)malloc(dwNum);//new wchar_t[dwNum];
	MultiByteToWideChar(0, 0, nge_screen->name, -1, wTitle, dwNum);

	hWnd = CreateWindowEx(WS_EX_WINDOWEDGE, winclass.lpszClassName, wTitle, dwStyle,
		rt.left, rt.top, rt.right - rt.left, rt.bottom - rt.top, 0, 0, hInstance, 0);
}

HWND GetNGEhWnd() { return hWnd; }
#endif
...
void InitGraphics()
{
	...
	if (nge_screen.name == NULL) {
		nge_screen.name = (char*)malloc(14);
		strcpy(nge_screen.name, "SINRGE2");
	}

#if defined NGE_WIN
	makeWindow(&nge_screen);
...
void FiniGraphics()
{
	...
	if (hInstance)
		UnregisterClassW(L"SINRGE2 Player", hInstance);
#endif
