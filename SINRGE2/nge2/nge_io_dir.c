...
static HANDLE st_FindFirstFile(const char* path)
{
	...
	dwNum = MultiByteToWideChar(0, 0, path, -1, NULL, 0);
	wPath = (wchar_t*)malloc(dwNum);
	MultiByteToWideChar(0, 0, path, -1, wPath, dwNum);

	handle = FindFirstFileW(wPath, &FindFileData);
	return handle;
}
