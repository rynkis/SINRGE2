...
{
	CFileDialog dlg(FALSE, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("7package archives (*.7pkg)|*.7pkg|All Files (*.*)|*.*||"));
	if (dlg.DoModal() != IDOK)
	{
		return;
	}
...
{
	CFileDialog dlg(TRUE, NULL, NULL, 0, _T("7pkg files (*.7pkg)|*.7pkg|All Files (*.*)|*.*||"));
	if (dlg.DoModal() != IDOK)
	{
		return;
	}
